...
function add({ username, server, characterName, nickname, nodeId }) {
  ...
  const profile = {
    ...
    pausedKeys: [],
    autoStart: false,
    // null = läuft lokal auf diesem Server (Standard, wie bisher). Gesetzt = die ID eines
    // gepairten Node-Agents (nodeRegistry.js), auf dem die CLI für diesen Account tatsächlich
    // läuft — siehe routes/profiles.js für die Weiterleitung.
    nodeId: nodeId || null,
...
// Weist ein bestehendes Profil einem Node zu (oder holt es mit nodeId=null zurück auf "lokal") —
// getrennt von add(), weil Node-Zuweisung nachträglich in der UI geändert werden kann und weil
// nodeRegistry beim Entfernen eines Nodes betroffene Profile automatisch zurücksetzt.
function setNode(id, nodeId) {
  const all = readAll();
  const profile = all.find(p => p.id === id);
  if (!profile) return null;
  profile.nodeId = nodeId || null;
  writeAll(all);
  return profile;
}
...
// vom tatsächlichen Prozesslauf abhängig, der bei jedem Dashboard-Neustart/-Update ohnehin
// verloren geht. Wird beim Start-Button auf true, beim Stop-Button auf false gesetzt; dient
// server.js beim Hochfahren dazu, zuletzt laufende Charaktere automatisch wieder zu starten.
function setAutoStart(id, autoStart) {
  const all = readAll();
  const profile = all.find(p => p.id === id);
  if (!profile) return null;
