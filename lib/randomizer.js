...
function generateDayPlan(username, dateStr, config, settings) {
...
function buildPlansForDate(dateStr) {
  ...
  for (const [username, config] of Object.entries(configs)) {
    ...
    raw[username] = generateDayPlan(username, dateStr, merged, settings);
  }
...
// "Account" = das Login (username), nicht der einzelne Charakter — ein Login kann mehrere
// Charaktere/Profile haben, die alle gemeinsam mit dem Login-Zeitplan starten/stoppen (gleiches
// Prinzip wie "Alle starten"/"Alle stoppen" auf der Accounts-Seite).
async function tick() {
  const settings = getSettings();
  ...
  for (const [username, plan] of Object.entries(plans)) {
    ...
    for (const member of members) {
      try {
        if (desired === 'idle') await stopProfileById(member.id);
        else await startProfileById(member.id);
      } catch (e) {
        // Best-effort: ein einzelner fehlgeschlagener Charakter darf weder die anderen
        // Charaktere desselben Logins noch den Tick für andere Accounts blockieren.
      }
    }
  }
