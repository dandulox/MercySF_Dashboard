...
async function tick() {
  ...
  for (const [username, plan] of Object.entries(plans)) {
    ...
    for (const member of members) {
      try { accountsRegistry.setNode(member.id, nodeId === 'local' ? null : nodeId); } catch (e) {
        console.error(`[randomizer] setNode fehlgeschlagen für ${username}/${member.id}:`, e.message);
      }
    }
