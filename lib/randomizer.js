...
const DEFAULT_SETTINGS = {
  timezone: 'UTC',           // IANA-Zeitzone (z. B. "Europe/Berlin") — dayStart/dayEnd/etc. gelten in DIESER Zeit, nicht der Server-Systemzeit
...
// Der Server (z. B. ein VPS) läuft oft in UTC, während dayStart/dayEnd/etc. sich der Nutzer in
// seiner eigenen Zeitzone denkt — ohne diese Umrechnung würde "06:00" schlicht "06:00 UTC"
// bedeuten, was je nach Standort mehrere Stunden von der tatsächlichen Absicht abweicht. Intl
// rechnet die Zielzone inkl. automatischer Sommerzeit-Umstellung um, ganz ohne eigene DST-Tabelle.
function nowInTimezone(timezone) {
  try {
    ...
    // Ungültige/unbekannte Zeitzone in den Einstellungen — auf Server-Zeit zurückfallen statt zu crashen.
    const d = new Date();
...
function todayDateStr() {
  const n = nowInTimezone(getSettings().timezone);
...
// Kernstück: Accounts werden auf die Pool-Nodes verteilt und dort NACHEINANDER eingeplant (nie
// überlappend) — ein Node kann zu einem Zeitpunkt nur eine VPN-Identität halten, zwei Accounts
// mit unterschiedlicher VPN-Config dürfen also nie gleichzeitig auf demselben Node laufen. Jeder
// Node führt seine eigene Warteschlange, unabhängig vom Tempo der anderen Nodes.
//
// Node-Wahl ist echtes Round-Robin MIT Kapazitätsprüfung, nicht reines Best-Fit nach Zeit: kein
// Node bekommt einen zweiten Account, bevor nicht jeder andere passende Node mindestens einen hat
// (dann keinen dritten, bevor nicht jeder mindestens einen zweiten hat, usw.) — sonst würde ein
// reiner "früheste freie Zeit"-Vergleich einzelne Nodes bevorzugen (z. B. wegen ihres zufälligen
// Start-Jitters) und überladen, während andere trotz freier Kapazität leer blieben. Ein Account
// wird nur dann als "heute nicht einplanbar" markiert, wenn WIRKLICH kein einziger Pool-Node mehr
// Platz hat.
function fillNodeQueues(queue, poolNodes, settings, dateStr) {
  ...
  const unscheduled = [];
  if (!poolNodes.length) return { plans, scheduledMeta, unscheduled };
  ...
  const nodeState = new Map(poolNodes.map(node => {
    ...
    // Kleiner Zufalls-Stagger (0 bis max. blockGapMinutes[1]) statt des vollen dayStart..dayEnd-
    // Bereichs — sonst würde ein ungünstig gewürfelter Node einen Großteil seines Tagesfensters
    // verlieren, bevor er überhaupt seinen ersten Account bekommt. dayEnd ist eine eigenständige,
    // vom Nutzer konfigurierte Einstellung und nicht als Jitter-Obergrenze gedacht.
    const stagger = Math.round(rand(rng, 0, settings.blockGapMinutes[1]));
    return [node.id, { cursor: dayStartMin + stagger, hasAccount: false, count: 0 }];
  }));
  ...
  for (const [username, config] of queue) {
    ...
    // Lücken zwischen den EIGENEN Blöcken vorab ziehen, damit die tatsächliche Gesamtdauer (inkl.
    // Lücken) schon VOR der Node-Wahl feststeht — sonst würde ein Account einem Node zugewiesen,
    // auf dem nur sein erster Block passt, und dort weit über hardEndMin hinauslaufen (sichtbar an
    // einer Node-Auslastung über 100%).
    const gaps = blockMinutes.slice(1).map(() => Math.round(rand(rng, settings.blockGapMinutes[0], settings.blockGapMinutes[1])));
    const totalSpan = blockMinutes.reduce((sum, m) => sum + m, 0) + gaps.reduce((sum, g) => sum + g, 0);
    ...
    for (const node of poolNodes) {
      ...
      const candidateStart = state.hasAccount ? state.cursor + settings.nodeHandoffMinutes : state.cursor;
      if (candidateStart + totalSpan > hardEndMin) continue; // die GANZE Blockfolge muss passen, nicht nur der erste Block
      // Erst nach Account-Anzahl sortieren (Round-Robin-Fairness), erst bei Gleichstand nach
      // frühester Startzeit — so wird jeder Node gleich oft bedient, statt dass reine Zeit-Nähe
      // einzelne Nodes systematisch bevorzugt.
      if (state.count < bestCount || (state.count === bestCount && candidateStart < bestStart)) {
        bestNode = node;
    ...
    if (!bestNode) {
      unscheduled.push({ username, reason: 'no_capacity' });
      continue; // wirklich kein Node hat heute noch Platz für diesen Account
    }
    const node = bestNode;
    const state = nodeState.get(node.id);

    let cursor = Math.round(bestStart);
    const blocks = [];
    for (let i = 0; i < blockMinutes.length; i++) {
      const bStart = cursor;
      const bEnd = bStart + blockMinutes[i];
      blocks.push({ start: bStart, end: bEnd });
      cursor = bEnd + (gaps[i] || 0);
    }
  ...
  return { plans, scheduledMeta, unscheduled };
}
...
function buildPlansForDate(dateStr) {
  ...
  if (!queue.length) return { plans: {}, unscheduled: [] };

  const { poolNodes, reserveNode } = todayNodePool(settings);
  if (!poolNodes.length) {
    // Häufigste Ursache: Single-Server-Setup, bei dem "Lokal" als Reserve-Node für Stadtwache
    // gesetzt wurde — dann bleibt kein Node mehr für die eigentlichen Bot-Blöcke übrig.
    console.error(`[randomizer] kein Pool-Node verfügbar (reserveNodeId=${settings.reserveNodeId || 'keiner'}) — ${queue.length} aktive(r) Account(s) bekommen heute KEINEN Bot-Block`);
    return { plans: {}, unscheduled: queue.map(([username]) => ({ username, reason: 'no_pool_node' })) };
  }
  const { plans, scheduledMeta, unscheduled } = fillNodeQueues(queue, poolNodes, settings, dateStr);
  if (reserveNode && scheduledMeta.length) {
    fillStadtwacheQueue(plans, scheduledMeta, reserveNode, settings, dateStr);
  }
  if (unscheduled.length) {
    console.error(`[randomizer] ${unscheduled.length} Account(s) heute nicht einplanbar (zu wenig freie Slots): ${unscheduled.map(u => u.username).join(', ')}`);
  }
  return { plans, unscheduled };
}

let cachedEpoch = null;
let cachedResult = { plans: {}, unscheduled: [] };

function plansForToday() {
  const epoch = planEpoch();
  if (cachedEpoch !== epoch) {
    cachedResult = buildPlansForDate(epoch);
    cachedEpoch = epoch;
  }
  return cachedResult.plans;
}

// Accounts, die heute trotz priorityQueue+Best-Fit keinen Node-Slot mehr bekommen haben (immer in
// derselben Reihenfolge wie priorityQueue, d. h. die niedrigst-priorisierten zuerst betroffen).
function unscheduledForToday() {
  plansForToday();
  return cachedResult.unscheduled;
}
...
function getStatusSummary() {
  ...
  const unscheduledCount = unscheduledForToday().length;

  return { enabledCount, totalAccounts, scheduledToday, remainingToday, unscheduledCount };
}
...
function getTimeline() {
  ...
  return {
    ...
    nowMinutes: currentMinutes(), // in der konfigurierten Zeitzone, NICHT der Browser-Zeit des Betrachters
    nodes,
    reserveNode: settings.reserveNodeId ? { id: settings.reserveNodeId, name: nodeName(settings.reserveNodeId), pulses } : null,
    unscheduled: unscheduledForToday().map(u => u.username),
  };
}
...
async function switchNodeVpn(nodeId, vpnMode, vpnProfileId) {
  ...
  try {
    ...
    console.error(`[randomizer] VPN-Trennen fehlgeschlagen auf Node ${nodeId}:`, e.message);
  }

  // vpnMode='profile' mit einer Profil-ID, die es nicht (mehr) gibt, darf NICHT einfach ohne
  // Gate-Änderung enden — sonst bliebe die vorherige Zuweisung (z. B. noch 'block') stehen und
  // der nachfolgende Start würde daran scheitern. Fällt in dem Fall explizit auf "kein VPN" zurück.
  const profile = vpnMode === 'profile' && vpnProfileId ? vpnProfiles.list().find(p => p.id === vpnProfileId) : null;
  if (vpnMode === 'profile' && vpnProfileId && !profile) {
    console.error(`[randomizer] VPN-Profil ${vpnProfileId} nicht gefunden — Node ${nodeId} läuft ohne VPN`);
  }
  ...
  try {
    ...
    console.error(`[randomizer] VPN-Zuweisung fehlgeschlagen auf Node ${nodeId}:`, e.message);
  }
}
...
async function tick() {
  ...
  const plans = plansForToday();
  const nowMinutes = currentMinutes();
  const { startProfileById, stopProfileById, assignProfileToNode } = require('../routes/profiles');
  ...
  for (const [username, plan] of Object.entries(plans)) {
    ...
    if (desired === 'idle') {
      if (isTransition || settings.hardEnforce) {
        for (const member of members) {
          try { await stopProfileById(member.id); } catch (e) {
            console.error(`[randomizer] stop fehlgeschlagen für ${username}/${member.id}:`, e.message);
          }
        }
      }
    ...
    if (!nodeId) {
      // Reserve-Node zwischenzeitlich entfernt — NICHT als erledigt markieren, sonst wird nie
      // wieder versucht zu starten, sobald wieder ein Reserve-Node gesetzt ist.
      console.error(`[randomizer] kein Node für ${username} (desired=${desired}) — reserveNodeId fehlt`);
      continue;
    }
    ...
    for (const member of members) {
      // Muss über assignProfileToNode laufen, nicht per accountsRegistry.setNode direkt — das
      // synct das Profil (Zugangsdaten inkl.) tatsächlich zum Node-Agent, sonst kennt der Node
      // den Account nicht und jeder startProfileById darunter scheitert mit "Profil nicht
      // gefunden", obwohl das Dashboard ihn schon als "auf Node X laufend" führt.
      if (member.nodeId === (nodeId === 'local' ? null : nodeId)) continue; // schon zugewiesen
      try { await assignProfileToNode(member.id, nodeId === 'local' ? null : nodeId); } catch (e) {
        console.error(`[randomizer] Node-Zuweisung fehlgeschlagen für ${username}/${member.id}:`, e.message);
      }
    }
    await switchNodeVpn(nodeId, plan.vpnMode, plan.vpnProfileId);
    for (const member of members) {
      try {
        await startProfileById(member.id);
      } catch (e) {
        console.error(`[randomizer] start fehlgeschlagen für ${username}/${member.id} auf Node ${nodeId}:`, e.message);
      }
    }
  }
...
setInterval(() => {
  tick().catch(e => console.error('[randomizer] Tick fehlgeschlagen:', e.message, e.stack));
}, TICK_INTERVAL_MS);

module.exports = {
  getSettings, setSettings,
  getAllConfigs, getConfig, setConfig,
  getTodayPlan, getStatusSummary, getTimeline, recalculate, unscheduledForToday,
  // für Tests:
