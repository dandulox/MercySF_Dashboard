...
//
// Node-Wahl ist Best-Fit (frühestmögliche freie Startzeit), nicht Round-Robin: ein Account wird
// nur dann als "heute nicht einplanbar" markiert, wenn WIRKLICH kein einziger Pool-Node mehr
// Platz hat — nicht schon, wenn zufällig genau der Node an der Reihe ist, den die
// Reihenfolge-Zählung ihm zugewiesen hätte, während ein anderer Node noch frei wäre.
function fillNodeQueues(queue, poolNodes, settings, dateStr) {
  ...
  for (const [username, config] of queue) {
    ...
    for (const node of poolNodes) {
      ...
      if (candidateStart < hardEndMin && candidateStart < bestStart) {
        bestNode = node;
